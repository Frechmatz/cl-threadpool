...
(defclass threadlist ()
  ...
  (:documentation
   "This class holds all threads generated by the thread pool. 
    Threads are added in two steps: 
    (1) Add a thread 'placeholder' that consists of a name
    (2) Attach a thread instance to the placeholder"))
...
(defun threadlist-attach-thread (threadlist thread-name thread)
  "Attaches a thread instance to an entry. Throws an error when the entry not
   exists or when it already is attached to a thread.
   - thread-name Name of thread as generated by threadlist-add-entry
   - thread A thread instance"
  (bt:with-lock-held ((slot-value threadlist 'lock))
    (let ((slot (assoc thread-name (slot-value threadlist 'threads) :test #'string=)))
      (if (not slot)
	  (error (format nil "Threadlist entry not found for name ~a" thread-name))
	  (if (not (eq (second slot) :pending))
	      (error (format nil "The threadlist entry ~a is already attached to a thread" thread-name))
	      (setf (second slot) thread))))))

(defun threadlist-remove-entry (threadlist thread-name)
  (bt:with-lock-held ((slot-value threadlist 'lock))
  ...
  (let ((thread-name (threadlist-add-entry (slot-value pool 'threads))))
	...
	(bt:current-thread))
	 ...
	 (loop
	    ...
	    (if (not (process))
		(return)))
	 (threadlist-remove-entry (slot-value pool 'threads) thread-name)
